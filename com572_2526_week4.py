# -*- coding: utf-8 -*-
"""COM572_2526_Week4.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1TGN5hAN9JMb08gfj0BjRE3GFyWLEVHTA
...
# Good for: High accuracy and handling non-linear data without much tuning.
...
# Good for: Pushing performance to the max (often wins competitions).
...
# Good for: Finding "look-alike" data points.
# Note: Scaling (StandardScaler) is MANDATORY here for the distance math to work.
...
dt_preds = evaluate('Decision Tree',     dt_pipeline, X_train, y_train, X_test, y_test)
gb_preds = evaluate('Gradient Boosting', gb_pipeline, X_train, y_train, X_test, y_test)
rf_preds = evaluate('Random Forest',     rf_pipeline, X_train, y_train, X_test, y_test)
knn_preds = evaluate('KNN', knn_pipeline, X_train, y_train, X_test, y_test)

"""## 6. Visualise Predictions"""

import matplotlib.pyplot as plt

# List of your pipelines and their names for easy iteration
models = [
    (lr_pipeline, 'Linear Regression'),
    (dt_pipeline, 'Decision Tree'),
    (rf_pipeline, 'Random Forest'),
    (gb_pipeline, 'Gradient Boosting'),
    (knn_pipeline, 'KNN Regressor')
]

# Create a 2-row, 3-column grid (6 total slots, we use 5)
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
axes = axes.flatten() # Flatten to 1D array for easy looping

for i, (pipe, title) in enumerate(models):
    ax = axes[i]
    ...
    # Plotting
    ax.scatter(y_test, preds, alpha=0.6, edgecolors='k', linewidths=0.5, color='royalblue')
    ...
    ax.set_title(f'{title}\n$R^2$: {pipe.score(X_test, y_test):.3f}')
    ax.legend()
...
# 1. Store all your pipelines in a dictionary
pipelines = {
...
# 2. Evaluate all models and store their R² scores
results = {}

for name, pipe in pipelines.items():
    # Ensure they are fitted (assuming X_train/y_train are defined)
    pipe.fit(X_train, y_train)
    preds = pipe.predict(X_test)
...
* `best_model.pkl` (or `.joblib`): Your trained Scikit-Learn model.
...
Would you like me to help you write a professional **README.md** to explain your model to people visiting your GitHub repository?
...
1. **Prepare your files**: Ensure your trained model file (`best_model.pkl`) is saved inside the `streamlit_lab` folder.
